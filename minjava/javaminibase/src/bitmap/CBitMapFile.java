	...
	implements GlobalConst
{
	 ...
	 * unique values mapped by this CBitMapFile. If it finds the value is already in
	 * the BTree it will return a RID with the PageNo set to the first page in the
	 * doubly linked CBMPage list for that value
	 */
		...
		throws IOException
	{
		...
		{
			System.out.println("Error in CBitMapFile::valueIsMapped()");
			ex.printStackTrace();
		}
	 ...
	 * to an existing PageId, and if it is then the method will return the CBitMapHeaderPage
	 * pointed to by that PageId
	 * If the RID.PageId is invalid, then we will create a new CBitMapHeaderPage for the value
	 * in the Value Class.
	 */
	public CBitMapHeaderPage findOrCreateHeader( ValueClass checkVal, RID listStartPage )
		throws ConstructPageException, IOException, HFException, HFBufMgrException,
				...
				InsertException
	{
		CBitMapHeaderPage pageToReturn = null;
		
		if( -1 == listStartPage.pageNo.pid )
		{
			//need a new page for this element value
			CBitMapHeaderPage newBMHeader = new CBitMapHeaderPage();
			//associate the new BMHeader Page to this RID
			listStartPage.pageNo.pid = newBMHeader.headCBMPageId.pid;
		...
		{
			//found matching page
			pageToReturn = new CBitMapHeaderPage( listStartPage.pageNo );
		}
	...
	/*
	 * Takes a Value & position. It will get/create a CBitMapHeaderPage
	 * for the doubly linked list set of CBMPages for this value and then
	 * call the insertMap() method, and that method will set the appropriate
	 * bit in the CBMPage to 1
	 */
	public boolean Insert( ValueClass value, int position )
	{
		...
		try
		{
			...
			CBitMapHeaderPage pageListToUse = findOrCreateHeader( value, listStartPage );
			successfulInsert = pageListToUse.insertMap( position );
		}
		catch( Exception ex )
		{
			System.out.println("Error in CBitMapFile::Insert()");
			ex.printStackTrace();
		}
	...
	/*
	 * This method will check if there is a linked list of CBMPages for
	 * the specified value. If there is it will clear the bit at the
	 * specified position in the appropriate CBMPage
	 */
	public boolean Delete( ValueClass value, int position )
	{
		...
		try
		{
			...
			{
				CBitMapHeaderPage pageListToUse = findOrCreateHeader( value, index );
				successfulDelete = pageListToUse.deleteMap( position );
			}
		}
		catch( Exception ex )
		{
			System.out.println("Error in CBitMapFile::Delete()");
			ex.printStackTrace();
		}
	...
	public boolean Update( ValueClass oldValue, ValueClass newValue, int position )
	{
		...
		{
			System.out.println("Error in CBitMapFile::Update()");
			ex.printStackTrace();
		}
