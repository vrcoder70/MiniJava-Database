...
public class HFPage extends Page
  ...
  public void init(PageId pageNo, Page apage)
	...
	//iterate a page's slots and check if the position matches the input
	//This method assumes each record in the HFPage is stored [ position(int), value(attribute) ]
	public int getSlotWithPosition( int Position, int posOffsetInData )
		throws IOException
	{
		int matchedSlot = -1;
		slotCnt = getSlotCnt();
		for(int sltNo = 0; sltNo < slotCnt; sltNo++)
		{
			int checkOffset = getSlotOffset( sltNo );
			int checkPosition = Convert.getIntValue(checkOffset + posOffsetInData, data);
			if( checkPosition == Position )
			{
				matchedSlot = sltNo;
				//quick exit for loop (no dupes should exist in Heapfile)
				sltNo = slotCnt;
			}
		}
		return matchedSlot;
	...
	/**
	 * sets the value of curPage to the page which contains the entry at the given position
	 * This method assumes each record in the HFPage is stored [ position(int), value(attribute) ]
	 */
	public void setCurPage_forGivenPosition( int Position )
		throws HFBufMgrException, IOException
	{
		//call name of same name below with 2nd argument of 0 offset
		setCurPage_forGivenPosition( Position, 0 );
		//Nothing to return. At this point the currentPage is on a page
		//with a match or is pointing to an invalid page
	}
	
	/**
	 * sets the value of curPage to the page which contains the entry at the given position
	 * This method allows for the "Position" to be at any offset in the record entry
	 */
	public void setCurPage_forGivenPosition( int Position, int posOffsetInData )
		throws HFBufMgrException, IOException
	{
		//This method is not necessarily being called from the
		//head of the Page list, so we need to implement it
		//in such a way it can scan forward and backward
		PageId currentPageId = curPage;
		PageId forwardPageId = nextPage;
		HFPage currentHFPage = new HFPage();
		boolean matchFound = false;
		
		//scan all preceding pages for the desired position value
		...
		while( keepChecking )
		{
			...
			pinPage(currentPageId, currentHFPage, false/*read disk*/);
			
			//iterate each entry in this page, checking for a position match
			if( -1 != getSlotWithPosition( Position, posOffsetInData ) )
			{
				matchFound = true;
				keepChecking = false;
			}
			
			//no match found on this page, iterate to previous page
			if( keepChecking )
			{
				PageId prevPageId = getPrevPage();
				//if we reach the end of the list, exit
				if( -1 == prevPageId.pid )
				{
					keepChecking = false;
				}
				//unpin current page in the buffer
				unpinPage(currentPageId, false /*not DIRTY*/);
				currentPageId = prevPageId;
			}
		}
		
		//if we didn't find the page while iterating in reverse, then
		//we need to iterate forward (checking our starting point had
		//a forward link)
		currentPageId = forwardPageId;
		if( (false == matchFound) && (-1 != currentPageId.pid) )
		{
			keepChecking = true;
		}
		//scan all succeeding pages from the original starting page
		while( keepChecking )
		{
			//pin page with this PageId for use, if it is in the buffer
			//it will be pinned otherwise it will be pulled into the buffer
			...
			//no match found on this page, iterate to next page
			if( keepChecking )
			{
