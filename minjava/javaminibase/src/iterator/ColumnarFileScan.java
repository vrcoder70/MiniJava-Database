...
public class ColumnarFileScan extends Iterator {
  ...
  /**
   * it is not part of the iterator interface, but we keep it public to
   * use it in the delete_query.
   * 
   * @param tid TID will be set the TID of the returned tuple
   * @return the result tuple
   * @exception JoinsException                 some join exception
   * @exception IOException                    I/O errors
   * @exception InvalidTupleSizeException      invalid tuple size
  ...
  public Tuple get_next(TID tid)
    ...
    while (true) {
      ...
      if (f.isTupleMarkedDeleted(tid.position)) {
        continue;
      }
